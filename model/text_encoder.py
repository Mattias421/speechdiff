...
class TextEncoder(BaseModule):
    def __init__(self, cfg):
        ...
        self.n_vocab = len(symbols) + 1
        self.n_feats = cfg.data.n_feats
        self.spk_emb_dim = cfg.model.spk_emb_dim
        self.n_spks = cfg.data.n_spks

        cfg = cfg.model.encoder
        self.n_channels = cfg.n_channels
        self.filter_channels = cfg.filter_channels
        self.filter_channels_dp = cfg.filter_channels_dp
        ...
        self.prenet = ConvReluNorm(cfg.n_channels, cfg.n_channels, cfg.n_channels, 
                                kernel_size=5, n_layers=3, p_dropout=0.5)

        self.encoder = Encoder(cfg.n_channels + (self.spk_emb_dim if self.n_spks > 1 else 0), cfg.filter_channels, cfg.n_heads, cfg.n_layers, 
                            cfg.kernel_size, cfg.p_dropout, window_size=cfg.window_size)

        self.proj_m = torch.nn.Conv1d(cfg.n_channels + (self.spk_emb_dim if self.n_spks > 1 else 0), self.n_feats, 1)
        self.proj_w = DurationPredictor(cfg.n_channels + (self.spk_emb_dim if self.n_spks > 1 else 0), cfg.filter_channels_dp, 
                                        cfg.kernel_size, cfg.p_dropout)
