...
class Diffusion(BaseModule):
    def __init__(self, cfg):
        ...
        cfg = cfg.model.decoder
        self.dim = cfg.dim
        self.beta_min = cfg.beta_min
        self.beta_max = cfg.beta_max
        self.pe_scale = cfg.pe_scale

        self.estimator = GradLogPEstimator2d(cfg.dim, 
    ...
    @torch.no_grad()
    def reverse_diffusion(self, z, mask, n_timesteps, stoc=False):
        h = 1.0 / n_timesteps
        xt = z * mask
        for i in range(n_timesteps):
            ...
            dxt = 0.5 * (xt - self.estimator(xt, mask, t))
            dxt = dxt * noise_t * h
            xt = (xt - dxt) * mask
        return xt
    ...
    def loss_t(self, x0, mask, t):
        ...
        time = t.unsqueeze(-1).unsqueeze(-1)
        cum_noise = get_noise(time, self.beta_min, self.beta_max, cumulative=True)
        noise_estimation = self.estimator(xt, mask, t)
        noise_estimation *= torch.sqrt(1.0 - torch.exp(-cum_noise))
        loss = torch.sum((noise_estimation + z)**2) / (torch.sum(mask)*self.n_feats)
        return loss, xt

    def compute_loss(self, x0, mask, offset=1e-5):
        t = torch.rand(x0.shape[0], dtype=x0.dtype, device=x0.device,
                       requires_grad=False)
        t = torch.clamp(t, offset, 1.0 - offset)
        return self.loss_t(x0, mask, t)
